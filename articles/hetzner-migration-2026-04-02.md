...
そもそも画像が重すぎてめちゃくちゃ大変だった。uploadsフォルダが4サイト合計で約1.8GB（一番重いサイトで1.1GB）。WebP変換やサムネイル再生成も検討したけど、今回の目的は「WPを使わない部分だけ静的にして速くする」ことなので、画像の最適化はやらないと判断した。CSS統合やミニファイも同じ理由でスキップ。静的配信にすればそれだけで十分速い。
